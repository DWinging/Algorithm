 ...
 * 시간 568 ms
 */
public class BJ_2268_97890267 {
    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        int n = Integer.parseInt(br.readLine());
        int[][] lines = inputLines(n, br);
        int[] parents = new int[n];
        int idx = lis(lines, parents, n);
        boolean[] visited = new boolean[n];
        int len = searchLIS(visited, parents, idx);
        System.out.print((n - len) + "\n" + buildString(visited, lines));
    }
    ...
    private static int lis(int[][] lines, int[] parents, int n) {
        int[] dp = new int[n];
        dp[0] = 0;
        parents[0] = -1;
        int cnt = 0;
    ...
    private static int searchLIS(boolean[] visited, int[] parents, int idx) {
        visited[idx] = true;
        if(parents[idx] == -1) return 1;
        else return searchLIS(visited, parents, parents[idx]) + 1;
    }

    private static String buildString(boolean[] visited, int[][] lines) {
        StringBuilder sb = new StringBuilder();
        for(int i = 0; i < visited.length; i++) {
            if(!visited[i]) sb.append(lines[i][0]).append("\n");
        }
        return sb.toString();
    }
}
