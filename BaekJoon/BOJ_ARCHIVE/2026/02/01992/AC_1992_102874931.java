...
class Main {
    ...
    private static void solve(char[][] arr, int s_y, int e_y, int s_x, int e_x) {
        char result = checkDots(arr, s_y, e_y, s_x, e_x);
        if(result != '2') {
            sb.append(result);
            return;
        }
        ...
        solve(arr, s_y, mid_y, s_x, mid_x); // 1구간
        solve(arr, s_y, mid_y, mid_x, e_x); // 2구간
        solve(arr, mid_y, e_y, s_x, mid_x); // 3구간
        solve(arr, mid_y, e_y, mid_x, e_x); // 4구간
