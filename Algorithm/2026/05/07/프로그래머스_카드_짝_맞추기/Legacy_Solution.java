class Solution {
    ...
    private void dfs(int[][] board, int r, int c, int cnt, int bit) {
        ...
        for(int b = 1; b <= KIND; b++) {
            ...
            searchCard(board, r, c, card1, card2, cnt, next, b);
            searchCard(board, r, c, card2, card1, cnt, next, b);
        }
    }
    
    private void searchCard(int[][] board, int r, int c, int[] card1, int[] card2, int cnt, int next, int b) {
        int val = bfs(board, r, c, card1);
        val += bfs(board, card1[0], card1[1], card2);
            
        boardMarking(board, card1, card2, 0);
        dfs(board, card2[0], card2[1], cnt + val, next);
        boardMarking(board, card1, card2, b);
    }
    
    private void boardMarking(int[][] board, int[] card1, int[] card2, int m) {
        board[card1[0]][card1[1]] = m;
        board[card2[0]][card2[1]] = m;
    }
    ...
    private int bfs(int[][] board, int r, int c, int[] target) {
        ...
        while(head < tail) {
            ...
            for(int[] d : DIST) {
                ...
                if(checkNextPoint(ny, nx, cnt, tail)) tail++;
    ...
    private boolean checkNextPoint(int y, int x, int cnt, int tail) {
        if(visited[y][x] < mark) {
            que[tail][0] = y;
            que[tail][1] = x;
            que[tail][2] = cnt + 1;
