...
class Main {
    ...
    private static void dijkstra(int start, int k) {
        ...
        while(!pq.isEmpty()) {
            ...
            for(Node node : edge[from]) {
                ...
                if(visited[next].size() >= k && nextCost + cost > visited[next].peek()) continue;
                visited[next].add(nextCost + cost);
                if(visited[next].size() > k) visited[next].poll();
    ...
    private static String buildString(int n, int k) {
        ...
        for(int i = 1; i <= n; i++) {
            sb.append(visited[i].size() == k ? visited[i].peek() : -1).append('\n');
        }
        return sb.toString();
    }
