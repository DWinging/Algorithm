...
public class Main {
    ...
    private static int solve(int n, int m, int h) {
        for(int i = 0; i <= 3; i++) {
            flag = false;
            backtracking(n, h, 1, 1, 0, -(m + 1), i);
            if(flag) return i;
        }
        return -1;
    }

    private static void backtracking(int n, int h, int y, int x, int stack, int idx, int max) {
        if(flag) return;
        if(stack == max) {
            flag = checkRoute(n);
            return;
        ...
        for(int i = y; i < n; i++) {
            ...
            for(int j = beginX; j <= h; j++) {
                ...
                if(labbers[i][j] == 0 && labbers[i + 1][j] == 0) {
                    labbers[i][j] = labbers[i + 1][j] =idx;
                    backtracking(n, h, i, j + 1, stack + 1, idx - 1, max);
